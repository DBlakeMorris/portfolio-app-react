...
const Section = ({ id, title, children, isHome = false }: { id: string; title: string; children: React.ReactNode; isHome?: boolean }) => {
  const { scrollYProgress } = useScroll()
  const opacity = useTransform(scrollYProgress, 
    [0, 0.2, 0.8, 1], 
    isHome ? [1, 1, 1, 1] : [0.8, 1, 1, 0.8]  // Adjusted opacity values
  )
...
export default function Portfolio() {
  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans relative">
      <ParticleBackground />
      <header className={`fixed top-0 left-0 right-0 ${isScrolled ? 'bg-black' : 'bg-transparent'} transition-colors duration-300 z-50`}>
        <nav className="container mx-auto px-4 py-4">
          <div className="flex justify-center items-center relative">
            <button
              className="lg:hidden absolute left-0 text-white z-50"
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            >
              {isMobileMenuOpen ? 'Close' : 'Menu'}
            </button>
            <ul className={`${isMobileMenuOpen ? 'flex' : 'hidden'} lg:flex flex-row justify-center items-center space-x-4 w-full lg:w-auto absolute lg:relative top-full left-0 right-0 bg-black lg:bg-transparent py-4 lg:py-0 z-40`}>
      ...
      <main className="relative z-20">
        <Section id="home" title="" isHome={true}>
          <div className="text-center">
            ...
            <p className="text-xl mb-8">ML & NLP Engineer</p>
            <motion.a
      ...
      <footer className="bg-black bg-opacity-70 text-white py-8 relative z-10">
        <div className="container mx-auto px-4 text-center">
          <p>
            Contact: danielblakemorris@gmail.com |{' '}
            <motion.a
              href="https://www.linkedin.com/in/daniel-blake-morris"
              target="_blank"
              rel="noopener noreferrer"
